...
import pathlib
base_dir_path = pathlib.Path('D:\\Multimodal\\GQA\\code\\multimodal-vqa\\MAC')


# Merge hdf5 files
lengths = [0]
with h5py.File(base_dir_path / "data/gqa_{name}.h5".format(name = args.name), 'w') as out:
	datasets  = {}
	...
	for i in tqdm(range(args.chunksNum)):
		with h5py.File(base_dir_path / "data/{name}/gqa_{name}_{index}.h5".format(name = args.name, index = i), 'r') as chunk: 
...
# Update info file
with open(base_dir_path / "data/{name}/gqa_{name}_info.json".format(name = args.name)) as infoIn:
	...
	with open(base_dir_path / "data/gqa_{name}_merged_info.json".format(name = args.name), "w") as infoOut:
