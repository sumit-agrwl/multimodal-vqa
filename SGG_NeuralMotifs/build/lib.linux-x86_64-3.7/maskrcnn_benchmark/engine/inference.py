...
def compute_on_dataset(model, data_loader, device, synchronize_gather=True, timer=None):
    ...
    # open train_subset sceneGraphs json file
    train_subset_sceneGraph_dir = "/home/ubuntu/GraphVQA/GraphVQA/sceneGraphs/trainsubset_attribute_sceneGraphs.json"
    with open(train_subset_sceneGraph_dir, 'r') as outfile:
        trainsubsetGraph = json.load(outfile)
    ...
    for _, batch in enumerate(tqdm(data_loader)):
        ...
        with torch.no_grad():
            ...
            if os.path.exists(os.path.join(cfg.DETECTED_SGG_DIR, 'custom_prediction_'+str(count)+'.json')):
                count += 1
                results_dict = {}
                continue
        ...
        if cfg.TEST.CUSTUM_EVAL:
            # import pdb; pdb.set_trace()
            save_predictions = _accumulate_predictions_from_multiple_gpus(results_dict, synchronize_gather=cfg.TEST.RELATION.SYNC_GATHER)
            detected_sgg = custom_sgg_post_precessing(save_predictions)
            with open(os.path.join(cfg.DETECTED_SGG_DIR, 'custom_prediction_'+str(count)+'.json'), 'w') as outfile:  
...
def inference(
    ...
    #         json.dump(detected_sgg, outfile)
    #     # with open(os.path.join(cfg.DETECTED_SGG_DIR, 'custom_prediction.json'), 'wb') as outfile: 
    #     #     pickle.dump(detected_sgg, outfile, pickle.HIGHEST_PROTOCOL)
