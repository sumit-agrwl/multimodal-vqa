...
class AnswerTable:
    ...
    def __init__(self, dsets=None):
        self.all_ans = json.load(open("answers/all_ans.json"))
...
def load_lxmert_qa(path, model, label2ans):
    ...
    #print (loaded_state_dict['answer_head.logit_fc.3.weight'].size())
    ...
    for key, value in loaded_state_dict.items():
        if key.startswith("answer_head."):
            answer_state_dict[key.replace('answer_head.logit_fc', 'coarse_logit_fc')] = value

    # print (model_state_dict.keys())
    # Do surgery on answer state dict
    ans_weight = answer_state_dict['coarse_logit_fc.3.weight']
    ans_bias = answer_state_dict['coarse_logit_fc.3.bias']
    import copy
    new_answer_weight = copy.deepcopy(model_state_dict['coarse_logit_fc.3.weight'])
    new_answer_bias = copy.deepcopy(model_state_dict['coarse_logit_fc.3.bias'])
    ...
    print()
    answer_state_dict['coarse_logit_fc.3.weight'] = new_answer_weight
    answer_state_dict['coarse_logit_fc.3.bias'] = new_answer_bias

    # Load Bert Weights
    bert_model_keys = set(model.coarse_encoder.model.state_dict().keys())
    bert_loaded_keys = set(bert_state_dict.keys())
    assert len(bert_model_keys - bert_loaded_keys) == 0
    model.coarse_encoder.model.load_state_dict(bert_state_dict, strict=False)
